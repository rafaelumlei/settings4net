...
This project provides a framework and tools that enable **.NET** applications to be configured in a centralized remote server[server](https://github.com/rafaelumlei/settings4all/tree/master/settings4all.API)) . The settings are implemented using normal code and no synchronization code is necessary.
...
* The settings are processed considering the priority of the repositories chain, the settings' values that come from high priority repositories override the ones that come from others;
